...
class Session:
    def __init__(self, lm, caps, tools, invariant, recorded: EventLog | None = None, prod_load=0,
        ...
        # Hosted-provider identity to bind on every llm_call (see receipt-spec: for a
        # hosted model, recording provider/model_version/system_fingerprint/response_id
        # is what makes provider drift distinguishable from tampering). None for a
        # local, recomputable model.
        self.provider = provider
        self.log = EventLog()
        rr = recorded_tool_results(recorded) if recorded is not None else None
        self.broker = Broker(caps, tools, self.log, replay=rr)

    def llm(self, prompt: str, n_new=8, provider: dict | None = None):
        ...
        payload = {"prompt_hash": hash_text(prompt), "tokens": toks,
                   "logit_hashes": hashes, "text": text}
        prov = {**(self.provider or {}), **(provider or {})}   # per-call overrides session default
        if prov:
            payload["provider"] = prov
        self.log.append(Kind.LLM_CALL, payload)
        return text
...
def record_run(agent_fn, lm, caps, tools, key, prog_hash, prod_load=8, invariant=True, priv=None,
               provider: dict | None = None):
    s = Session(lm, caps, tools, invariant, recorded=None, prod_load=prod_load, provider=provider)
    agent_fn(s)
