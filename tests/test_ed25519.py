...
def test_signature_verifies_against_the_embedded_public_key_only(signed):
    ...
    assert verify_certificate(cert, log)["sig_valid"] is True
...
def test_corrupted_signature_is_rejected(signed):
    ...
    assert verify_certificate(forged, log)["sig_valid"] is False
...
def test_resigned_with_a_different_key_fails_a_pinned_anchor(signed):
    ...
    assert verify_certificate(resigned, log)["sig_valid"] is True
...
def test_hmac_fallback_round_trip_and_wrong_key(agent_log):
    ...
    assert verify_certificate(cert, agent_log, key=key)["sig_valid"] is True
    assert verify_certificate(cert, agent_log, key=b"wrong-key")["sig_valid"] is False
