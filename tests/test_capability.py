...
def test_granted_tool_runs_and_is_recorded_as_allow():
    exfil = []
    log = EventLog()
    b = Broker(CAPS, make_tools(exfil), log, allow_cleartext=True)
...
def test_denied_call_is_recorded_without_a_result():
    log = EventLog()
    b = Broker(CAPS, make_tools([]), log, allow_cleartext=True)
    b.call("read_secret")
...
def test_alias_of_a_forbidden_tool_is_still_ungranted():
    ...
    b = Broker(CAPS, tools, log, allow_cleartext=True)
...
def test_recorded_tool_results_returns_allow_results_in_order():
    exfil = []
    log = EventLog()
    b = Broker({"read_public"}, make_tools(exfil), log, allow_cleartext=True)
...
def test_replay_reinjects_results_and_never_reruns_live_tools():
    ...
    exfil = []
    rec_log = EventLog()
    rb = Broker({"read_public"}, make_tools(exfil), rec_log, allow_cleartext=True)
    ...
    rpb = Broker({"read_public"}, {"read_public": boom, "read_secret": boom,
                                   "send_external": boom}, rep_log, replay=list(recorded), allow_cleartext=True)
